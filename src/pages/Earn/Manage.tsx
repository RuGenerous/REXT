...
import { JSBI, TokenAmount, CurrencyAmount, CAVAX, Token, WAVAX, ChainId } from '@rugenerous/sdk'
...
import { BIG_INT_ZERO, RUG } from '../../constants'
...
export default function Manage({
  ...
  const [, avaxRugTokenPair] = usePair(CAVAX, RUG[chainId ? chainId : 43114])
  if (avaxPool) {
    ...
    let rug
    if (tokenA && tokenA.equals(RUG[tokenA.chainId])) {
      token = tokenB
      rug = tokenA
    } else {
      token = tokenA
    ...
    if (totalSupplyOfStakingToken && stakingTokenPair && avaxRugTokenPair && tokenB && rug) {
      const oneToken = JSBI.BigInt(1000000000000000000)
      const avaxRugRatio = JSBI.divide(
        JSBI.multiply(oneToken, avaxRugTokenPair.reserveOf(WAVAX[tokenB.chainId]).raw),
        avaxRugTokenPair.reserveOf(rug).raw
      )

      const valueOfRugInAvax = JSBI.divide(JSBI.multiply(stakingTokenPair.reserveOf(rug).raw, avaxRugRatio), oneToken)

      valueOfTotalStakedAmountInWavax = new TokenAmount(
        WAVAX[tokenB.chainId],
        JSBI.divide(
          JSBI.multiply(
            JSBI.multiply(stakingInfo.totalStakedAmount.raw, valueOfRugInAvax),
            JSBI.BigInt(2) // this is b/c the value of LP shares are ~double the value of the wavax they entitle owner to
          ),
          totalSupplyOfStakingToken.raw
        )
        ...
        <PoolData>
          <AutoColumn gap="sm">
            ...
            <TYPE.body fontSize={24} fontWeight={500}>
              ...
              {t('earnPage.rewardPerWeek', { symbol: 'RUG' })}
            </TYPE.body>
          </AutoColumn>
        </PoolData>
      </DataRow>
        ...
        <BottomSection gap="lg" justify="center">
          ...
          <StyledBottomCard dim={stakingInfo?.stakedAmount?.equalTo(JSBI.BigInt(0))}>
            ...
            <AutoColumn gap="sm">
              <RowBetween>
                <div>
                  <TYPE.black>{t('earnPage.unclaimedReward', { symbol: 'RUG' })}</TYPE.black>
                </div>
