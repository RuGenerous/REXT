...
import { JSBI } from '@rugenerous/sdk'
    ...
    if (stakingInfos?.length > 0) {
      Promise.all(
        stakingInfos
          ...
          .map(stakingInfo => {
            return fetch(`https://api.rugenerous.exchange/rugenerous/apr/${stakingInfo.stakingRewardAddress}`)
              .then(res => res.json())
              .then(res => ({
      ...
      <TopSection gap="md">
        <DataCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{t('earnPage.rugenerousLiquidityMining')}</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>{t('earnPage.depositRugenerousLiquidity')}</TYPE.white>
              </RowBetween>{' '}
              <AutoRow justify="space-between">
                <ExternalLink
                  style={{ color: 'white', textDecoration: 'underline' }}
                  href="https://rugenerous.exchange/litepaper"
                  target="_blank"
                >
                  <TYPE.white fontSize={14}>{t('earnPage.readMoreAboutRug')}</TYPE.white>
                </ExternalLink>
                <FlexDiv>
                  <ExternalLink
          ...
          <DataCard>
            ...
            <CardSection>
              <AutoColumn gap="md">
                ...
                <RowBetween>
                  <TYPE.white fontSize={14}>{t('earnPage.rugenerousGovernanceProposalResult')}</TYPE.white>
                </RowBetween>
                {version !== '0' && (
                  <NavLink style={{ color: 'white', textDecoration: 'underline' }} to="/rug/0">
                    <TYPE.white fontSize={14}>{t('earnPage.oldRugPools')}</TYPE.white>
                  </NavLink>
                )}
              </AutoColumn>
            </CardSection>
