    ...
    Promise.all(
      stakingInfos
        ?.sort(function(info_a, info_b) {
          // greater stake in rug comes first
          return info_a.totalStakedInRug?.greaterThan(info_b.totalStakedInRug ?? BIG_INT_ZERO) ? -1 : 1
        })
        .sort(function(info_a, info_b) {
      ...
      <TopSection gap="md">
        <DataCard>
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{t('earnPage.rugenerousLiquidityStaking')}</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>{t('earnPage.depositRugenerousStaking')}</TYPE.white>
              </RowBetween>
            </AutoColumn>
          </CardSection>
        </DataCard>
