...
import { RUG, ZERO_ADDRESS } from '../../constants'
import { JSBI, TokenAmount, ChainId } from '@rugenerous/sdk'
...
export default function Vote() {
  ...
  const rugBalance: TokenAmount | undefined = useTokenBalance(account ?? undefined, chainId ? RUG[chainId] : undefined)
  const userDelegatee: string | undefined = useUserDelegatee()

  // show delegation option if they have have a balance, but have not delegated
  const showUnlockVoting = Boolean(
    rugBalance && JSBI.notEqual(rugBalance.raw, JSBI.BigInt(0)) && userDelegatee === ZERO_ADDRESS
  )
      ...
      <TopSection gap="md">
        <VoteCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{t('votePage.rugenerousGovernance')}</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>{t('votePage.earnedRugTokens')}</TYPE.white>
              </RowBetween>
              <RowBetween>
      ...
      <TopSection gap="2px">
        <WrapSmall>
          ...
          {showUnlockVoting ? (
            ...
            JSBI.notEqual(JSBI.BigInt(0), rugBalance?.raw) ? (
            <TYPE.body fontWeight={500} mr="6px">
              <FormattedCurrencyAmount currencyAmount={rugBalance} /> {t('votePage.votes')}
            </TYPE.body>
          ) : (
            ''
          )}
