...
import { TokenAmount } from '@rugenerous/sdk'
import { useActiveWeb3React } from '../../hooks'
import { maxAmountSpend } from '../../utils/maxAmountSpend'
import { useRugContract, useStakingContract } from '../../hooks/useContract'
...
export default function StakingModalSingleSide({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  const stakingTokenContract = useRugContract()
        ...
        stakingContract
	        ...
	        .then((response: TransactionResponse) => {
		        addTransaction(response, {
			        summary: t('earnPage.stakeStakingTokens', { symbol: 'RUG' })
		        })
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            <TYPE.largeHeader>{t('earn.depositingToken', { symbol: 'RUG' })}</TYPE.largeHeader>
            <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} RUG</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>{t('earn.deposited')} {parsedAmount?.toSignificant(4)} RUG</TYPE.body>
          </AutoColumn>
        </SubmittedView>
      )}
    </Modal>
