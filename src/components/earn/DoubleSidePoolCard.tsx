...
import { CAVAX, Token } from '@rugenerous/sdk'
  ...
  const token: Token =
    currency0 === CAVAX || currency1 === CAVAX
      ? currency0 === CAVAX
        ? token1
        : token0
      : token0.equals(RUG[token0.chainId])
      ? token1
      : token0
    ...
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        {(isStaking || !stakingInfo.isPeriodFinished) && (
          <StyledInternalLink
            to={`/rug/${currencyId(currency0)}/${currencyId(currency1)}/${version}`}
            style={{ width: '100%' }}
          >
            <ButtonPrimary padding="8px" borderRadius="8px">
      ...
      <AprContainer>
        ...
        <RowBetween>
          <TYPE.white>RUG Rewards APR</TYPE.white>
          ...
          <BottomSection showBackground={true}>
            ...
            <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
              ...
              {`${stakingInfo.rewardRate
                ?.multiply(`${60 * 60 * 24 * 7}`)
                ?.toSignificant(4, { groupSeparator: ',' })} RUG / week`}
            </TYPE.black>
          </BottomSection>
        </>
      )}
