...
export default function DelegateModal({ isOpen, onDismiss, title }: VoteModalProps) {
  ...
  const rugBalance = useTokenBalance(account ?? undefined, chainId ? RUG[chainId] : undefined)
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          <AutoColumn gap="lg" justify="center">
            ...
            <TYPE.body>{t('vote.earnedRug')}</TYPE.body>
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOndismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.main fontSize={36}>{rugBalance?.toSignificant(4)}</TYPE.main>
          </AutoColumn>
        </LoadingView>
      )}
      {hash && (
