import { ChainId, TokenAmount } from '@rugenerous/sdk'
...
const RUGAmount = styled(AccountElement)`
  color: white;
  padding: 4px 8px;
  height: 36px;
  font-weight: 500;
...
export default function Header() {
  ...
  const aggregateBalance: TokenAmount | undefined = useAggregateRugBalance()

  const [showRugBalanceModal, setShowRugBalanceModal] = useState(false)
  ...
  return (
    <HeaderFrame>
      <Modal isOpen={showRugBalanceModal} onDismiss={() => setShowRugBalanceModal(false)}>
        <RugBalanceContent setShowRugBalanceModal={setShowRugBalanceModal} />
      </Modal>
      <HeaderRow>
        <Title href=".">
          <RugIcon>
      ...
      <HeaderControls>
        <HeaderElement>
          ...
          {aggregateBalance && (
            <RUGWrapper onClick={() => setShowRugBalanceModal(true)}>
              <RUGAmount active={!!account} style={{ pointerEvents: 'auto' }}>
                {account && (
                  <HideSmall>
                    <TYPE.white
                      style={{
