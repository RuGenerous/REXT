import { ChainId, Token } from '@rugenerous/sdk'
import { Tags, TokenInfo, TokenList } from '@rugenerous/token-lists'
...
export function useIsSelectedAEBToken(): Boolean {
  ...
  // ignore RUG and WAVAX token
  if (aebToken?.address === RUG[ChainId.AVALANCHE].address || aebToken?.address === WAVAX[ChainId.AVALANCHE].address) {
    return false
  }
